%The main structure of the simulation script
clear all
clc
...
Td=24 + 3.*randn;%Division point of a cell
Tr=6.8;%Replication point of a cell
...
molecule=2;%2 molecules are produces per generation(~23 min) on average
k= 0.1214;%(calculated by the kscript)
...
while RT < DT
%time point of molecule generation
r=rand;
delt= -log(1-r)/k;%Production of a new molecules 
RT = RT + delt;
B1(deltcount,1)=delt;% to keep the record of data
...
elseif (CA >= Tr && CA < Td)
    CM=CM+1;
    k=0.0607;
    %Rate of production of new molecules gets double
    %k and delta(production of new molecules) are inversely proportional
elseif (CA >= Td)%condition after division
    k=0.1214;
    Td=24 + 3.*randn;% 24 mins of normallly distributed time/generation
    A1(index+1,5)=Td;
    Tr=6.8; %We could make it dependent of Td(more complicated)
    A1(index+1,4)=Tr;
    deltSum=CA;
    A1(index,6)=deltSum;%summation of delt for a perticular generation
    CA=0;%Return to the bith of a cell
...
xlabel('Daughter 1')
ylabel('Daughter 2')
Avg = mean(FinalResultMatrix,1); % Average of this simuation

%save('SimDataR.mat','U');if I like save the raw matrix

%matrix modification if we want to cobind similar distribution patterns
%uU = triu(U);
%lU = triu(U');
%tU = lU + uU;
%tU  = tU([1:size(tU,1)],[1:size(tU,2)])./2;
%figure
%bar3(tU)
%xlabel('Daughter 1')
%ylabel('Daughter 2')
